...
### BUG-EVT-001: Bookタブ並び変更時にキー遷移と装備選択が連鎖的に崩れる（再発）

**発生日**: 2026-04-24  
**重要度**: 高

#### 症状
- タブの見た目順を変更しても、左右キー/QEの遷移順が古いまま残る
- ステータスタブで装備変更中、選択中の項目と確定される項目がずれることがある
- 「なし」選択時に、ルアーは外れるがエサ装備が残るケースがある

#### 原因
`src/scenes/GameScene.ts` 内で、Bookタブ順序とステータス装備ナビ状態が複数箇所に分散管理されていた。

1. タブ順序配列が複数のハンドラにハードコードされており、DOM順を変えてもキー処理側が自動で追従しなかった  
2. ステータス装備確定ロジックで、`equipmentId === null`（「なし」）時の早期return条件が `equippedLureId` のみを見ており、`equippedBaitId` が考慮漏れだった  
3. UI変更（DOM順）と入力状態遷移（キー操作）の整合を検証する回帰テストがなく、部分修正で再発した

**問題箇所**: `src/scenes/GameScene.ts`（Bookタブ遷移分岐群、`equipFromStatusPanel()`）

#### 解決法
- Bookタブ順序を `unifiedBookTabOrder` に一元化し、左右/QE/端遷移の全ルートで同一配列を参照
- `equipFromStatusPanel()` の「なし」分岐を修正し、`bait/lure` の両方が未装備のときのみreturn、それ以外は両方解除
- 装備解除メッセージを実際の挙動に合わせて更新（`エサ/ルアーを外した`）

#### 教訓・対策
- **単一責務化**: ナビ順序や遷移規則は1つの定数/関数に集約し、ハードコードの重複を禁止する
- **状態遷移の網羅チェック**: DOM順変更時は「見た目」「キー遷移」「確定処理」を同時に確認する
- **早期return条件のレビュー**: `null` 分岐や解除処理では、関連状態（今回なら `bait/lure`）を必ず対で確認する
- **回帰テスト項目化**: 「タブ順変更後に左右キー順が一致する」「ステータスで“なし”が常に正しく解除される」をテスト観点として固定する

#### 関連ファイル
- `src/scenes/GameScene.ts`: Bookタブ順序、キー遷移、ステータス装備選択・確定処理
- `src/style.css`: タブ見た目順の確認対象（構造変更時の副作用確認）
...
### ファイル別
- `src/scenes/GameScene.ts`: BUG-UI-001, BUG-EVT-001
